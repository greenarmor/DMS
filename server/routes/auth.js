...
router.post("/register", async (req, res) => {
  ...
  const user = await User.create({ email, publicKey, lastLogin: new Date() });
...
router.post("/webauthn/register", async (req, res) => {
  ...
  const options = {
    ...
    authenticatorSelection: {
      authenticatorAttachment: "platform", // This ensures the use of passkeys
...
router.post("/webauthn/verify", async (req, res) => {
  ...
  if (
    ...
    req.session.challenge !== clientDataJSON.challenge
  ) {
