...
cron.schedule('0 0 * * *', async () => {
  const now = new Date();
  const schedules = await Schedule.findAll({ where: { nextCheck: { [Op.lte]: now } }, include: User });

  schedules.forEach(schedule => {
    if (new Date(schedule.User.lastLogin).getTime() < now.getTime() - (3 * 30 * 24 * 60 * 60 * 1000)) {
      sendEmail(schedule.recipientEmail, schedule.subject, schedule.body, schedule.attachment);
      schedule.nextCheck = new Date(now.getTime() + (3 * 30 * 24 * 60 * 60 * 1000)); // 3 months later
      schedule.save();
    }
